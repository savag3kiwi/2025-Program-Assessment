...
# Currency Formatting for the original cheese frame
add_dollars = ['Cheese Price']
...
while budget <= MAX_SPEND:
    ...
    try:
        ...
        if budget <= 19:
            print(f"Sorry {name}, your budget is now less than the cheapest "
                  f"Cheese ($20). Thanks for shopping!")
            print()
            break
...
# Currency Formatting for the purchased frame
add_dollars = ['Cheese Price']
for var_item in add_dollars:
    purchased_frame[var_item] = purchased_frame[var_item].apply(currency)
